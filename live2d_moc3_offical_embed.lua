-- live2d_moc3_offical_embed.lua - windowless official Cubism Core runtime for host apps.
--
-- Hosts can load Cubism 3/4/5 model resources from files or memory streams,
-- drive parameters/motions, inspect official Core drawable data, and render with
-- live2d.cubism3_offical.renderer when an OpenGL context is current.

package.path = package.path .. ";./?.lua;./?/init.lua"

local Model = require("live2d.cubism3_offical.model")
local GLRenderer = require("live2d.cubism3_offical.renderer")
...
local function require_model(self)
    if self.model == nil then
        error("official Cubism model is not loaded", 3)
    end
    return self.model
end
...
function Renderer:load_model(model_path, opts)
    ...
    self.model = Model.new(moc_bytes)
...
function Renderer:get_drawables()
    return require_model(self):getAllDrawableData()
end

function Renderer:get_meshes()
    return self:get_drawables()
end
...
function Renderer:get_parameter(param_id)
    return require_model(self):getParameterValueById(param_id)
end

function Renderer:get_parameter_by_index(index)
    return require_model(self):getParameterValue(tonumber(index) or 1)
end

function Renderer:set_parameter(param_id, value)
    local model = require_model(self)
    if model.param_id_to_index[param_id] == nil then
        error("unknown parameter: " .. tostring(param_id), 2)
    end
    model:setParameterValueById(param_id, tonumber(value) or 0)
    return self
end

function Renderer:set_parameter_by_index(index, value)
    index = tonumber(index) or 1
    if index < 1 or index > require_model(self).parameter_count then
        error("unknown parameter index: " .. tostring(index), 2)
    end
    require_model(self):setParameterValue(index, tonumber(value) or 0)
    return self
end

function Renderer:reset_parameters()
    require_model(self):resetParameters()
    return self
end

function Renderer:set_part_opacity(part_id, value)
    local model = require_model(self)
    if model.part_id_to_index[part_id] == nil then
        error("unknown part: " .. tostring(part_id), 2)
    end
    model:setPartOpacityById(part_id, tonumber(value) or 0)
    return self
end

function Renderer:set_part_opacity_by_index(index, value)
    index = tonumber(index) or 1
    if index < 1 or index > require_model(self).part_count then
        error("unknown part index: " .. tostring(index), 2)
    end
    require_model(self):setPartOpacity(index, tonumber(value) or 0)
    return self
end

function Renderer:reset_part_opacities()
    require_model(self):resetPartOpacities()
    return self
end
...
function Renderer:start_motion(group, no, weight)
    ...
    player.weight = tonumber(weight) or 1.0
...
function Renderer:get_gl_renderer()
    if self.gl_renderer == nil then
        ...
        self.gl_renderer = GLRenderer.new(self.gl)
    end
    return self.gl_renderer
end

function Renderer:render(projection, texture_paths)
    return self:get_gl_renderer():renderModel(require_model(self), texture_paths or self:get_texture_paths(), projection)
end
...
function M.get_drawables()
    if current_renderer == nil then error("no current renderer", 2) end
    return current_renderer:get_drawables()
end
