"""PySide6 + lupa host for the official Cubism Core Rana renderer.

Run from the repository root:

    python examples/pyside6_lupa_rana_offical.py
...
standard Lua instead of LuaJIT, this example cannot load the official Core path
because the project depends on LuaJIT FFI.
"""
...
class Live2DWidget(QOpenGLWidget):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Live2D Rana - PySide6 + lupa - Official Core")
    ...
    def initializeGL(self) -> None:
        ...
        try:
            ...
            self.embed = self.lua.execute(b'return require("live2d_moc3_offical_embed")')
            self._load_model = self.lua.eval(
                b"function(embed, model_path, opts) "
            ...
            self._start_motion = self.lua.eval(
                b"function(embed, no) "
                b"local groups = {'mtn_idle01_C', 'mtn_smile01_C', 'mtn_nod01_C'}; "
                b"local group = groups[(no % #groups) + 1]; "
                b"return embed.start_motion(group, 0, 1.0) "
                b"end"
            )
            ...
            self.lua.execute(
                ...
                b"local canvas = embed.current():get_model().canvas; "
            ...
            self._load_model(self.embed, MODEL_PATH.encode("utf-8"), opts)
    ...
    def _show_error(self, message: str) -> None:
        QMessageBox.critical(
            self,
            "Live2D official Core initialization failed",
            "Failed to initialize Rana Live2D through lupa.\n\n" + message,
        )
